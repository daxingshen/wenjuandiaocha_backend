...
不装 Go/pnpm/node/pg,一条命令把 **数据库 + 后端 + 前端两端** 整体拉起。前端构建上下文是相邻的
UI 仓,**须两仓并列 checkout**(`../wenjuandiaocha_ui`)。
...
docker compose stop           # 暂停(保留容器与数据)
docker compose start          # 恢复
docker compose down           # 删容器(数据卷 pgdata 保留,重启数据不丢)
