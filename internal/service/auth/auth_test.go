...
func TestLogin_WrongPassword_SameAsUnknownAccount(t *testing.T) {
	...
	f := &fakeStore{user: dao.User{UserID: "u1", PasswordHash: hash, Name: "n", Role: "creator"}}
	m := New(f, time.Hour)
...
func TestLogin_Success_CreatesSession(t *testing.T) {
	...
	f := &fakeStore{user: dao.User{UserID: "u1", PasswordHash: hash, Name: "Alice", Role: "creator"}}
	m := New(f, time.Hour)
